...
class Classify:
    ...
    def getLinkResult(self):
        ...
        return self.__linkresult
    
    def getPairMap(self,level):
    ...
    def getSingleNum(self,index,dataMatrix):
        ...
        for _group in self.__pairList:
            ...
            _reslist.append("%5.3f" % (_res / _num))
        return _reslist
    ...
    def ComputeResult(self,userlist,dataMatrix,level):
        #self.getLinkResult()
        _pairMap = self.getPairMap(level)
        self.__pairList = []
        for _key in _pairMap:
            self.__pairList.append(_pairMap[_key])
        self.getShowMatrix(userlist, dataMatrix)
        return self.__showMatrix
...
if __name__=='__main__':
    ...
    c = Classify(data)
    user = ['a','b','c','d','e','f']
    print c.ComputeResult(user,d,3)
